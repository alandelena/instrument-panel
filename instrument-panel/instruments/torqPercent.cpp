...
torqPercent::torqPercent(int xPos, int yPos, int size, const char* parentName) : instrument(xPos, yPos, size)
{
    if (parentName) {
...
void torqPercent::resize()
{
    ...
    ALLEGRO_BITMAP* orig = loadBitmap("torq-percent.png");
    addBitmap(orig);
...
void torqPercent::update()
{
    ...
    if (simVars->turbineTorquePercent < 60) {
        // The 0-60% arc spans 135 degrees
        angle = 135 * (simVars->turbineTorquePercent / 60 );
    }
    else {
